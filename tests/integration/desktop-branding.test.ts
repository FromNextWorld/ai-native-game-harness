...
describe('AI Native Game Harness branding', () => {
  ...
  it('opens the original Harness by default and keeps explicit two-way navigation', () => {
    ...
    const gameUi = read('apps/desktop/src/game-ui-plugin/client.js')
    expect(gameUi).toContain("name: 'settings.section'")
    expect(gameUi).toContain("'ai-native-game-harness://game'")
    expect(gameUi).toContain("'ai-native-game-harness://evaluation'")
    expect(main).not.toContain("position: 'fixed'")
    ...
    expect(main).toContain("label: '游戏'")
