...
- 公共仓库只包含协议、SDK、Adapter、通用插件、Desktop 基础能力和可复现验收；账号、额度、支付与托管 Provider 路由属于独立产品服务，不进入公共源码。
- 产品发行层只能单向消费固定的公共 Release；公共构建不得引用、拉取或内嵌私有仓库，游戏 MOD 也不得接触服务器密钥。

### 一个 Harness，可选官方托管插件

- 公共开发源码仍可独立运行，使用开发者自己的模型与工具。公共构建不安装或依赖官方账号、支付和计费实现。
- 官方服务发行层预装可选插件，在「设置 → 插件 → 官方托管服务」提供勾选项，新安装默认启用；取消后使用原有「模型」设置配置自己的服务，不再提供独立的服务模式切换。
- 勾选项控制实际插件加载与官方模型接管，不只是隐藏账号界面。停用时阻止新的官方请求，重启后撤去官方模型覆盖；已经被上游接受的工作不承诺撤销，原会话若明确绑定官方模型需重新选择可用模型。
- 反复启用/停用保持同一聊天、工作区和运行数据路径。旧版分开的数据目录只保留并固定当前使用的一套，不自动混合、删除或承诺另一套记录已迁入。
- 以上描述官方服务发行层的行为，不表示公共开发包内置商业服务，也不代表生产支付、签名安装包或长期真实游戏体验已验收。
- 旧仓库 `dsh-xiaotangyuan-game` 只保留历史和旧 Release，不再维护第二套源码。
- `1.0` 分支与 `v1.0.0` 标签继续固定第一份稳定源码；`1.1` 分支与 `v1.1.0` 标签固定本次稳定源码。不能移动既有标签或用开发提交覆盖稳定分支。
- 封版提交的根工作区版本为 `1.1.0`；合并后 `main` 已进入 `1.2.0-dev.0` 开发线。各插件、Adapter 与 Bridge 继续独立版本。
