...
describe('game runtime configuration', () => {
  ...
  it('defaults to mandatory multimodal and speech capabilities without embedding secrets', () => {
    ...
    expect(config.speech.iflytekAppIdCredentialRef).toBe('IFLYTEK_APP_ID')
    expect(config.speech.iflytekApiKeyCredentialRef).toBe('IFLYTEK_API_KEY')
    expect(config.speech.iflytekApiSecretCredentialRef).toBe('IFLYTEK_API_SECRET')
