...
> 仓库说明：本文随小汤圆源码迁入 `ai-native-game-harness`。下方指向 `dsh-xiaotangyuan-game` 的链接仅用于下载已经发布的历史安装包；新源码和后续版本只在主仓库维护。
...
文字对话和游戏适配器不依赖具体模型厂商。公共标准版内置火山引擎 ASR/TTS，并同时提供用户自配凭据的科大讯飞流式 ASR；商业托管网关不是使用科大讯飞的前提。游戏会话由 DSH 中支持图片输入的模型直接接收玩家文字、截图和统一结构化 Context 并回答，不再串联第二个对话模型。
